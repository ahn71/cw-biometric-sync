...
def main():
    ...
    try:
        ...
        print("last_lift_off_timestamp->",last_lift_off_timestamp)
        print("now time->",datetime.datetime.now())
        print("PULL_FREQUENCY->",datetime.timedelta(minutes=config.PULL_FREQUENCY))
        print("Compare to->",datetime.datetime.now() - datetime.timedelta(minutes=config.PULL_FREQUENCY))
        if (last_lift_off_timestamp and last_lift_off_timestamp < datetime.datetime.now() - datetime.timedelta(minutes=config.PULL_FREQUENCY)) or not last_lift_off_timestamp:
            ...
            for device in config.devices:
                ...
                try:
                    print('before pull') 
                    print('device_attendance_logs->>',device_attendance_logs)
                    ...
                    print('exception when calling pull_process_and_push_data function for device')
...
def pull_process_and_push_data(device, device_attendance_logs=None):
    ...
    for device_attendance_log in device_attendance_logs[index_of_last+1:]:
        ...
        print("Befor send user_id >>"+device_attendance_log['user_id'])        
        status_code, message = send_attendance(device_attendance_log['user_id'],device_attendance_log['timestamp'],device['device_id'],punch_direction,latitude=device['latitude'],longitude=device['longitude'])
        if status_code == 200:
            attendance_success_logger.info("\t".join([message, str(device_attendance_log['uid']),
            ...
            attendance_failed_logger.error("\t".join([str(status_code), str(device_attendance_log['uid']),
                str(device_attendance_log['user_id']), str(device_attendance_log['timestamp'].timestamp()),
                str(device_attendance_log['punch']), str(device_attendance_log['status']),
                json.dumps(device_attendance_log, default=str)]))
            if not(any(error in message for error in allowlisted_errors)):
...
def send_to_cwhrms(
    ...
    try:
        url = f"{config.CWHRMS_URL}/api/Attendance/attendace/biometric/create"
...
def send_attendance(
        ...
        latitude=None,
        longitude=None):
    print("in send_attendance")
    print("HR_SYSTEM: ",config.HR_SYSTEM.upper())

    if config.HR_SYSTEM.upper() == "ERPNext".upper():
    ...
    elif config.HR_SYSTEM.upper() == "CWHRMS".upper():
